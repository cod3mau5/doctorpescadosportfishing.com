...
<div class="container-fluid px-0">
            ...
            <!-- panga_form -->
            <div class="row px-0">
                <form class="col-12 mb-4" method="POST" action="" enctype='multipart/form-data' id="panga-form">
                        ...
                        <div class="row">
                            ...
                            <div class="file-field">
                                <div class="z-depth-1-half mb-4">
                                    <img :src="panga.img ? panga.img : 'https://mdbootstrap.com/img/Photos/Others/placeholder.jpg'" class="img-fluid"
                                    alt="example placeholder" v-show="!panga.img_preview && panga.img">
                                    
                                </div>
                                <div class="d-flex justify-content-center">
                                    <div class="btn btn-mdb-color btn-rounded" :class="{ 'alert-red': alert.img_panga }" style="max-width:100%">
                                        <span>Subir Imagen Principal de la Panga</span> <br>
                                        <input type="file" name="img" id="imgPanga" style="max-width:100%;overflow:hidden!important" placeholder="Seleccionar Imagen" v-on:change="previewFiles" @click="alert.img_panga = false">
                                    </div>
                                </div>
                            </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group"  :class="{ 'alert-red': alert.panga_errors.name }">
                                    <label for="name">Nombre de la panga</label>
                                    <input name="name" class="form-control form-control-lg" type="text" placeholder="Escribe aqui el nombre de la panga" id="name" v-model="panga.name"  @click="alert.panga_errors.name = false">
                                </div>
                            </div>
                        </div>
                        <!-- panga_max_anglers -->
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group" :class="{ 'alert-red': alert.panga_errors.max_anglers }">
                                    <label for="max_anglers" class="mb-1 mb-sm-2" >Capacidad maxima (personas a bordo)</label>
                                    <input type="number" name="max_anglers" class="form-control" id="max_anglers" v-model="panga.max_anglers" 
                                    @click="alert.panga_errors.max_anglers = false">
                                </div>
                            </div>
                        
                            <div class="col-6">
                                <div class="form-group" :class="{ 'alert-red': alert.panga_errors.feets }">
                                    <label for="feets" class="mb-3 mb-sm-2"># Pies </label>
                                    <input type="number" name="feets" class="form-control" id="feets" v-model="panga.feets" @click="alert.panga_errors.feets = false">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group" :class="{ 'alert-red': alert.panga_errors.description }">

                                    <label for="description">Descripción (opcional)  </label>
            
                                    <textarea name="description" class="form-control" id="description" rows="3" v-model="panga.description" @click="alert.panga_errors.description = false"></textarea>

                                </div>
                                    <button type="submit" class="btn btn-info float-right mt-2 py-2 px-4" style="font-size:18px;font-weight:bolder" id="submitPanga" v-show="!editing_panga">Subir Panga</button>
                                    <button type="submit" class="btn btn-edit float-right mt-2 py-2 px-4" style="font-size:18px;font-weight:bolder" id="editPanga" v-show="editing_panga">Editar Panga</button>
                        
                            </div>
                        </div>

                        <input type="hidden" v-model="panga.id">
                            ...
                            <form class="row" action="" method="POST" id="boatSpecificationsForm">   
                                ...
                                <div class="col-6 px-1 py-1">
                                    <ul id="fishing_gear" class="dynamic-list">
                                        <input v-for="(item, index) of boatSpecs.fishing_gear" :key="index" :value="item" :id="item" name="fishing_gear[]" class="mb-1">  
                                    </ul>
                                </div>
                                ...
                                <div class="col-6 px-1 py-1">
                                    <ul id="boat_features" class="dynamic-list">
                                        <input v-for="(item, index) of boatSpecs.boat_features" :key="index" :value="item" :id="item" name="boat_features[]" class="mb-1">
                                    </ul>
                                ...
                                <input type="hidden" name="panga_id" v-model="panga.id">
                ...
                <div class="col-12 navbarSon2"> 
                    ...
                    <div class="collapse navbar-collapse-burguer not-show col-12" id="navbarSupportedContent42">
                        ...
                        <div id="navbarSon2" class="my-3">
                                ...
                                <div class="row">
                                    <div class="col-6 col-sm-4 mb-3 card" v-for="(pangaImage, index) of pangaImages" :key="index" v-show="!deletedPangaImg.includes(pangaImage.id)">
                                        <img :src="$props.url+'/img/pangas/gallery/'+pangaImage.name" class="w-100"  >
                                        <button class="small btn btn-outline-danger p-1 mt-1" @click="deleteImgBoat(pangaImage.id)">Eliminar</button>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4 offset-md-4">
                                        <div class="file-field">
                                            ...
                                            <div class="btn btn-mdb-color btn-rounded" style="max-width:100%;" :class="{ 'alert-red': alert.img_captain }">
                                                ...
                                                <input type="file" name="captain_img" id="captainImageUploader" style="max-width:100%;overflow:hidden!important" @click="alert.img_captain = false">
                                            </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    export default {
        data: function(){
            return {
                ...
                panga:JSON.parse(this.$props.this_panga),
                pangaImages:JSON.parse(this.$props.panga_images),
                deletedPangaImg:[],
                captain:JSON.parse(this.$props.panga_captain),
                rates:JSON.parse(this.$props.panga_rates),
                boatSpecs:{
                    fishing_gear: JSON.parse(this.$props.boat_specs).fishing_gear.split(','),
                    boat_features: JSON.parse(this.$props.boat_specs).boat_features.split(',')
                },
            }
        },

        mounted() {
            console.log('we are on panga-edit.vue');
            if(this.boatSpecs.fishing_gear[this.boatSpecs.fishing_gear.length -1] == ''){
                this.boatSpecs.fishing_gear.pop();
            }
            if(this.boatSpecs.boat_features[this.boatSpecs.boat_features.length -1] == ''){
                this.boatSpecs.boat_features.pop();
            }
            ...
            $('#submitPanga').click((event) => {
                ...
                if (typeof files !== "undefined") {
                    ...
                    fd.append('_token', vm.$props.csrf_token);
                        ...
                        success: function(response) {
                            if (response.action == "success") {
                                ...
                                vm.panga.id = response.panga_id;
                                vm.panga.img_preview = response.panga_img;
                                vm.editing_panga = true

                                $.blockUI({
                                    message: '<p style="font-weight:bolder;font-size:3em;text-align:center;color:#ffffff;line-height:1;margin-bottom:.3rem">Panga <span style="color:#7400b8">' + response.panga_name + '</span> creada!<br> ahora puedes seguir editando más propiedades de la panga<span style="color:#7400b8">...</span></p>',
                                    css: {
                                        width: '650px',
                                        border: 'none',
                                        padding: '15px',
                                ...
                                vm.alert.panga_errors = response;
                            }
                        }
                    ...
                    $.blockUI({
                        message: '<p style="font-weight:bolder;font-size:3em;text-align:center;color:#ffffff;line-height:1;margin-bottom:.3rem">Se necesita subir la imagen principal<br>de la panga!</p>',
                        css: {
                            width: '650px',
                            border: 'none',
                            padding: '15px',
                    ...
                    vm.alert.img_panga = true;
                    $('html, body').animate({
                        scrollTop: $('#panga-form').offset().top
                    }, 910);
                }

            });

            $('#editPanga').click((event) => {
                event.preventDefault();
                var vm = this;
                var files = $('#imgPanga')[0].files[0];
                var panga_id = vm.panga.id;
                if (typeof files !== "undefined") {
                    var fd = new FormData();
                    fd.append('img', files);
                    fd.append('name', vm.panga.name);
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.action == "success") {
                                vm.boat_features = true;
                                vm.panga.id = response.panga_id;
                                vm.panga.img = response.panga_img;
                                vm.editing_panga = true

                                $.blockUI({
                                    message: '<p style="font-weight:bolder;font-size:3em;text-align:center;color:#28a745;line-height:1;margin-bottom:.3rem">Panga ' + response.panga_name + ' editada!</p>',
                                    css: {
                                        width: '650px',
                                        border: 'none',
                                        padding: '15px',
            ...
            $('#submitCaptain').click((event) =>  {
                    ...
                    fd.append('panga_id', vm.panga.id);
                    fd.append('_token', vm.$props.csrf_token);

                    if (vm.captain.captain_img){
            ...
            $('#editCaptain').click((event) =>  {
                ...
                if (vm.captain.captain_img){
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/admin/update-captain-info/'+panga_id,
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: fd,
                        success: function(response) {
                            if (response.action == "success") {
                                ...
                                vm.editing_captain=true;
                                vm.captain.captain_img=response.captain_img;
                                console.log(response);
                                setTimeout(function() {
                                    expandFunction(event, 'navbarSupportedContent43', 'navbarSon3');
                                    $.unblockUI();
                                ...
                                let unorderList='';
                                if(response.captain_img){unorderList=unorderList+'<li>'+response.captain_img +'</li>'}
                                if(response.captain_name){unorderList=unorderList+'<li>'+response.captain_name +'</li>'}
                                if(response.captain_experience){unorderList=unorderList+'<li>'+response.captain_experience+'</li>'}
                                if(response.captain_origin){unorderList=unorderList+'<li>'+response.captain_origin+'</li>'}
                                $.blockUI({
            ...
            $('#editRates').click(()=>{
                ...
                // console.log(rates_data);
                        $.ajax({
                            type: "POST",
                            url: '/admin/update-rates/'+panga_id,
                            data: rates_data,
                            success: function(response) {
                                if (response.action == "success") {
                                    $.blockUI({
        ...
        methods:{
            ...
            updateBoatSpecs:function(){
                    ...
                    var vm1 = this;
                    var panga_id = vm1.panga.id;
                    $.ajax({
                        type: "POST",
                        url: '/admin/edit-boat-specification/'+panga_id,
                        dataType: 'json',
                        data: data,
                        success: function(response) {
                            if (response.action == "success") {
            ...
            sendingEvent: function(){
                ...
                var panga_id= this.panga.id;
                this.editDropzone= new Dropzone(this.$refs.imageUpload, {
                    url: '/admin/image_boats',
            ...
            deleteImgBoat:function(id){
                ...
                $.ajax({
                    type: "POST",
                    url: '/admin/image-boats-delete/'+id,
                    data: data,
                    success: function(response) {
                        if (response.action == "success") {
