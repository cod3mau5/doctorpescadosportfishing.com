...
@section('content')
    ...
    @if(auth()->user()->role == '1') 
        ...
        <a href="{{route('charters.create')}}" id="create-report" class="btn btn-success float-right" data-toggle="tooltip" data-placement="top" title="" data-original-title="Crear Nueva Lancha">
                        ...
                        <div class="card_image" style="background-image: url('{{ url($charter->img) }}')!important;"></div>
                            <div class="card-body p-2">
                                <h5 class="card-title mb-1">
                                    {{$charter->name}}  <small>{{$charter->feets}}</small>                                   
                                </h5>
                            @if($charter->rate)
                                <a href="{{route('charters.show',[$charter->id])}}" class="btn btn-primary mb-2 px-3 w-100 text-center" 
                                    data-toggle="tooltip" 
                                    data-placement="top" 
                                    title="" 
                                    data-original-title="Ver La Lancha">
                            ...
                            @endif
                                <form method="POST" action="{{route('charters.destroy',[$charter->id])}}" class="d-inline">
                                ...
                                <a href="{{route('charters.edit',[$charter->id])}}" class="btn btn-info px-3" data-toggle="tooltip" data-placement="top" title="" data-original-title="Editar" style="font-size: 12px;width:40%">
                                    Editar
